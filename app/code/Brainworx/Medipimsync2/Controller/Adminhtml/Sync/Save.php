...
// 				}
			...
			if($update || $insert){
				...
				$cat->setStoreId($this->_storemanager->getStore('nl')->getId())->setName((string)$incat->translation_nl)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_nl))->save();
				
				$cat->setStoreId($this->_storemanager->getStore('fr')->getId())->setName((string)$incat->translation_fr)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_fr))->save();
				
				$cat->setStoreId($this->_storemanager->getStore('en')->getId())->setName((string)$incat->translation_en)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_en))->save();
				
// 				$cat->setStoreId($this->_storemanager->getStore('nl')->getId())->setName((string)$incat->translation_nl)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_nl))->setIsActive(1)->save();
				
// 				$cat->setStoreId($this->_storemanager->getStore('fr')->getId())->setName((string)$incat->translation_fr)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_fr))->setIsActive(1)->save();
				
// 				$cat->setStoreId($this->_storemanager->getStore('en')->getId())->setName((string)$incat->translation_en)->setUrlKey(preg_replace('/\s+/', '_', (string)$incat->translation_en))->setIsActive(1)->save();
